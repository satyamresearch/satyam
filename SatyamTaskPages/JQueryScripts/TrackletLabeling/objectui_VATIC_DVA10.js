function TrackObjectUI(button, container, videoframe, job, player, tracks)
{
    ...
    //as soon as you click new object, this notification happens
    ...
    //this is the first function that is called once the images are loaded
    
    this.setup = function()
    {
        //set up all the event notifications
        this.button.button({
            icons: {
            ...
            me.startnewobject(); //as soon as someone clicks on the screen a new object starts
        });
        ...
        // var html = "<p>In this video, please track all the different kinds of objects in the list below.  that lie within the closed regions indicated by the red lines:</p>";
        var html = "<h3>Instructions:</h3><li><font color='red'>Select ALL</font> actions <font color='red'>THAT APPLY, ONLY WHEN</font> the object in the bounding box perform those actions.</li>" +
            "<li>Make sure to <font color='red'>UNSELECT</font> as the actions disappear and <font color='red'>SELECT</font> new actions when they appear as the video plays forward. </li>";
        this.instructions = $(html).prependTo(this.container);
        ...
        //}
        //html += "</ul>";
        //html += "<p>Example Good Work</p>"
        //html += '<iframe width="200" src="https://www.youtube.com/embed/KC8A_jz78h0" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
...
function TrackObject(job, player, container, color)
{
    ...
    this.updateboxtext = function()
    {
        ...
        {
            var retval = this.track.estimateattribute(i, this.player.frame);
            if (retval)
            {
                str += "<br>";
        ...
        var v = $("#annotateoptionshideboxtext")[0];
    ...
    //this is the function that adds the details box - krishna comment
    this.setupdetails = function()
    {
        ...
        //this.details.append("<input type='checkbox' id='trackobject" + this.id + "occluded'> <label for='trackobject" + this.id + "occluded'>Temporarily occluded (less than 80% visible)</label><br>");
        this.details.append("<input type='checkbox' style='visibility:hidden' id='trackobject" + this.id + "lost'> <label for='trackobject" + this.id + "lost'  style='visibility:hidden'>Outside of view frame</label><br>");
        this.details.append("<input type='checkbox' style='visibility:hidden' id='trackobject" + this.id + "occluded'> <label for='trackobject" + this.id + "occluded' style='visibility:hidden'>Temporarily occluded (less than 80% visible)</label><br>");
        this.details.append("<strong>Select all that apply: </strong>")

        for (var i in this.job.attributes[this.track.label]) {
            this.details.append(
                "<br><label for='trackobject" + this.id + "attribute" + i + "' ><strong> " + this.job.attributes[this.track.label][i] + "</strong></label ><br>"
                + "<input type='radio' value = 'true' name = 'trackobject" + this.id + "attribute" + i + "' id='trackobject" + this.id + "attribute" + i + "_True'> True"
                + "<input type='radio' value = 'false' name = 'trackobject" + this.id + "attribute" + i + "' id='trackobject" + this.id + "attribute" + i + "_False'> False<br>"
            );

            // create a closure on attributeid
            (function (attributeid) {
                $("input:radio[name='trackobject" + me.id + "attribute" + attributeid + "']").click(function () {
                    me.player.pause();
                    var checked = this.value;
                    me.track.setattribute(attributeid, checked =="true"? true : false);
    ...
    this.updatecheckboxes = function()
    {
        ...
        var ob1 = $("#trackobject" + this.id + "lost")[0];
        if (ob1) {
            ob1.checked = e.outside;
        }
        //$("#trackobject" + this.id + "lost").attr("checked", e.outside);
        
        var ob2 = $("#trackobject" + this.id + "occluded")[0];
        if (ob2) {
            ob2.checked = e.occluded;
        }
        ...
        {
            var retval = this.track.estimateattribute(i, this.player.frame); 
            var ob = $("input:radio[name='trackobject" + this.id + "attribute" + i + "']:checked");
            if (ob.length>0)
            {
                if (!retval) {
                    //ob[1].checked = true;
                    $("#trackobject" + this.id + "attribute" + i + "_False")[0].checked = true;
                } else {
                    //ob[0].checked = true;
                    $("#trackobject" + this.id + "attribute" + i + "_True")[0].checked = true;
                }
            }
        }
        //this.updateboxtext();
    ...
    this.showtooltip = function(onscreen)
    {
        ...
        var update = function() {
            ...
            me.tooltip.css("background-image", "url('" + me.job.frameURLList[frame] + "')");
