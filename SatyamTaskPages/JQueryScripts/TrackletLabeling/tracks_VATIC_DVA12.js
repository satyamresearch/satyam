...
function BoxDrawer(container)
{
    ...
    this.enable = function()
    {
        ...
        this.hcrosshair.css({
            ...
            width: '100%',
            position: 'relative',
            top: '-22px', //this is a hack and needs fixing
            left: '0px',
            backgroundColor: this.color,
            zIndex: 1
        }).hide();
    ...
    this.updatecrosshairs = function(visible, xc, yc)
    {
        if (this.enabled)
        {
            if (visible && !this.drawing)
            {
                ...
                //this.hcrosshair.show().css('top', (yc - 22) + 'px'); //this is a hack and needs fixing
...
function TrackCollection(player, job)
{
    ...
    this.serialize = function () {
        ...
        for (var i in this.tracks) {
            if (!this.tracks[i].deleted) {
                if (!first) {
                    str += "|" + this.tracks[i].serialize(this.job.labels);
                } else {
                    str = this.tracks[i].serialize(this.job.labels);
                    first = false;
                }
...
function Track(player, color, position)
{
    ...
    this.journal = new Journal(player.job.start, player.job.blowradius, player.job.scalingFactor);
    ...
    this.initAttributesWithDefaultFalseFirstFrame = function(attributes)
    {
        for (var i in attributes)
        {
            var journal = new Journal(this.player.job.start, 
    ...
    this.draw = function(frame, position)
    {
       ...
       /* this.handle.css({
        ...
        var hideboxoption = $("#annotateoptionshideboxes")[0];
        if (hideboxoption.checked == true) {
            this.handle.hide();
        }
    }
    ...
    this.serialize = function (labels) {
        if (this.deleted) {
            return "";
        }
        var str = labels[this.label] + "_" + this.journal.serialize(); 
        for (var i in this.attributejournals) {
            str += "_" + this.player.job.attributes[this.label][i] + "_" + this.attributejournals[i].serialize();
        }
        return str;
    }
 ...
 */
function Journal(start, blowradius, scalingFactor)
{
    this.annotations = {};
    ...
    this.serialize = function () {
        ...
        var str;
         for (var frame in this.annotations)
         {
            ...
            {
                // dat = dat.serialize();
                dat = dat.serialize(this.scalingFactor);
            }
            if (!first) {
                str += ":" + frame + "," + dat;
            }
        ...
        //this is for stationary objects that dont move out of the view
        //even till the end
         if (this.artificialrightframe != null && this.annotations[this.artificialrightframe] == null) {
             var dat = this.artificialright;
             if (dat instanceof Object) {
                 dat = dat.serialize(this.scalingFactor); //passing 1 since it was already scled before
             }
